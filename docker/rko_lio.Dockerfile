...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,target=/var/lib/apt,sharing=locked \
    apt-get update \
 && (apt-get install -y ros-${ROS_DISTRO}-rko-lio && echo "rko_lio installed from apt") \
 || (echo "apt package ros-${ROS_DISTRO}-rko-lio not found — building from source" \
     ...
     && colcon build --packages-select rko_lio --cmake-args -DRKO_LIO_FETCH_CONTENT_DEPS=ON -DCMAKE_BUILD_TYPE=Release -DCMAKE_POLICY_VERSION_MINIMUM=3.5)
